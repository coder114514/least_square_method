...
struct Matrix
{
	...
	{
		data.resize(n+1);
		for (int i = 1; i <= n; i++) data[i].resize(m+1);
	}
...
Matrix inv(const Matrix& M)
{
	...
	for (int i = 1; i <= M.m; i++)
	{
		...
		for (int k = 1; k <= M.m; k++)
		{
			...
			for (int j = i; j <= 2*M.m; j++) W[k][j] -= ep*W[i][j];
		}
		double v = W[i][i];
		for (int j = 1; j <= 2*M.m; j++) W[i][j] /= v;
	}
	Matrix res(M.m, M.m);
	for (int i = 1; i <= M.m; i++)
		for (int j = 1; j <= M.m; j++)
